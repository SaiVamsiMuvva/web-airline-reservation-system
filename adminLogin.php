...
session_start();
	if (($_POST['Login'])) {
		...
		if($user && $password){
			...
			$query=mysqli_query($con,"SELECT * FROM Admins WHERE Email_id ='$user'");
        ...
        $dbadminname= $row['Admin_name'];
				...
				echo "<script>alert('Admin doesnot Exist!!');</script>";
			mysqli_close($con);
		}
		else
			echo "<script>alert('Admin Name or Password fields are missing');</script>";
	}
	else
		echo $form;
?>
...
<div class="container-fluid bg-3 text-center" style="padding-top: 100px;
  ...
  <div class="row">
    ...
    <form method="post">
    <p>Email id&nbsp;&nbsp;<input type="email" name="AdminEmail" id="AdminEmail" required></p>
